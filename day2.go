...
func day2() {
	games := `Game 1: 1 blue, 1 red; 10 red; 8 red, 1 blue, 1 green; 1 green, 5 blue
	...
	gameSlice := strings.Split(games, "\n")
	total := 0
	for _, game := range gameSlice {
		min := map[string]int{
			"red":   0,
			"blue":  0,
			"green": 0,
		...
		for i := 0; i < len(game); i++ {
			val := 0
			// converting string to int from mixed string
			for game[i] >= '0' && game[i] <= '9' {
				val = val*10 + int(game[i]-'0')
				i++
			}
			if game[i] == ' ' && val != 0 {
				i++
			...
			if game[i] == 'r' {
				min["red"] = max(val, min["red"])
				i += 4
			} else if game[i] == 'b' {
				min["blue"] = max(val, min["blue"])
				i += 5
			} else if game[i] == 'g' {
				min["green"] = max(val, min["green"])
				i += 6
			}
		}
	...
	// for game 1: 2439
	// var verified bool
	// for i := strings.Index(game, ":") + 2; i < len(game)-3; i++ {
	...
	// 			} else if game[i+1] > '4' {
	// 				verified = true
	// 			} else if game[i+3] != 'b' && game[i+1] > '3' {
	// 				verified = true
	// 			} else if (game[i+3] != 'g' && game[i+3] != 'b') && game[i+1] > '2' {
	// 				verified = true
	// 			}
	// 		}
	// 	}

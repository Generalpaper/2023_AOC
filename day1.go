...
func day1() {
	...
	for _, word := range wordSlice {
		...
		for j := len(word) - 1; num2 == 11; j-- {
			if word[j] >= '0' && word[j] <= '9' {
				num2 = int(word[j]) - 48
			} else {
				num2 = reverseCheck(word, j+1)
			}
		}
		total += num1*10 + num2
	}
...
func check(word string, ind int) int {
	length := len(word)
	if length-ind > 3 {
		if word[ind:ind+3] == "one" {
			return 1
		} else if word[ind:ind+3] == "two" {
			return 2
		} else if word[ind:ind+3] == "six" {
			return 6
		}
		if length-ind > 4 {
			if word[ind:ind+4] == "four" {
				return 4
			} else if word[ind:ind+4] == "five" {
				return 5
			} else if word[ind:ind+4] == "nine" {
				return 9
			}
			if length-ind > 5 {
				if word[ind:ind+5] == "three" {
					return 3
				} else if word[ind:ind+5] == "seven" {
					return 7
				} else if word[ind:ind+5] == "eight" {
					return 8
				}
			}
		}
...
func reverseCheck(word string, ind int) int {
	if ind >= 5 {
		if word[ind-5:ind] == "three" {
			return 3
		} else if word[ind-5:ind] == "seven" {
			return 7
		} else if word[ind-5:ind] == "eight" {
			return 8
		}
	}
	if ind >= 4 {
		if word[ind-4:ind] == "four" {
			return 4
		} else if word[ind-4:ind] == "five" {
			return 5
		} else if word[ind-4:ind] == "nine" {
			return 9
		}
	}
	if ind >= 3 {
		if word[ind-3:ind] == "one" {
			return 1
		} else if word[ind-3:ind] == "two" {
			return 2
		} else if word[ind-3:ind] == "six" {
			return 6
		}
	}
	return 11

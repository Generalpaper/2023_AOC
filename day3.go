...
func day3() {
	engine := `...........................775.651...............887....79...946...921...493.....942..942.....151....155....................................
	......240...................*.....-......................$..*...................*.......%.....+....................956.549.*290.......834...
	...
	engineRows := strings.Split(engine, "\n\t")
	// fmt.Println(len(engineRows[0]))
	total := 0
	gearCoordinates := make(map[[2]int]int)
	for i, row := range engineRows {
		for j := 0; j < len(row); j++ {
			verified := false
			number := 0
			coords := [2]int{}
			for j < len(row) && row[j] >= '0' && row[j] <= '9' {
				number = number*10 + int(row[j]-'0')
				collection := getAdjacent(engineRows, i, j)
				if collection[8] != '.' || collection[9] != '.' {
					coords = [2]int{int(collection[8]), int(collection[9])}
					verified = true
				}
				j++
			}
			...
			if verified == true {
				if v, yes := gearCoordinates[coords]; yes == true {
					total += v * number
				} else {
					gearCoordinates[coords] = number
				}
			}
		}
	}
	fmt.Println(total)
	// 82711149 part 2 too low
	// works for example input tho so idk
}

func isValidPos(i, j, m, n int) bool {
...
func getAdjacent(arr []string, i, j int) [10]byte {
	...
	if isValidPos(i-1, j-1, n, m) {
		if arr[i-1][j-1] == '*' {
			v[8], v[9] = byte(i-1), byte(j-1)
		}
		v[0] = arr[i-1][j-1]
	}
	if isValidPos(i-1, j, n, m) {
		if arr[i-1][j] == '*' {
			v[8], v[9] = byte(i-1), byte(j)
		}
		v[1] = arr[i-1][j]
	}
	if isValidPos(i-1, j+1, n, m) == true {
		if arr[i-1][j+1] == '*' {
			v[8], v[9] = byte(i-1), byte(j+1)
		}
		v[2] = arr[i-1][j+1]
...
// func getSpot (i, j int) byte {
// 	if j < 0 || j >= 140 {
// 		return '.'
// 	}
// 	if i < 0 || i >= len(engineRows[j]) {
// 		return '.'
// 	}
// 	return engineRows
// }
